...
export default function ChatPage({ params }: { params: Promise<{ discipline: string }> }) {
  ...
  useEffect(() => {
    ...
    const getUser = async () => {
      ...
      // Show language banner if not dismissed
      if (!localStorage.getItem('aquaref_lang_banner_v1')) {
  ...
  const handleFeedback = async (index: number, feedback: 'like' | 'dislike') => {
    ...
    await supabase.from('answer_feedback').insert({
      user_email: user?.email, discipline, question, answer: message.content, feedback
    })
  }
  ...
  const sendMessage = async () => {
    ...
    try {
      const response = await fetch('/api/chat', {
        ...
        body: JSON.stringify({ question, discipline, userEmail: user?.email })
      })
      ...
      if (response.status === 429) {
        if (data.error === 'monthly_limit_reached') {
          ...
          setMessages([...newMessages, { role: 'assistant', content: data.message, feedback: null }])
        } else {
          setMessages([...newMessages, { role: 'assistant', content: data.error, feedback: null }])
        }
      } else if (data.error) {
        setMessages([...newMessages, { role: 'assistant', content: data.error, feedback: null }])
      } else {
        setMessages([...newMessages, { role: 'assistant', content: data.answer, feedback: null }])
      ...
      setMessages([...newMessages, { role: 'assistant', content: 'Something went wrong. Please try again.', feedback: null }])
    }
    setLoading(false)
  }
  ...
  const getSampleQuestions = () => {
    const questions: { [key: string]: string[] } = {
      swimming: [
        ...
        'Apakah peraturan pusingan breaststroke?',
        '仰泳转身规则是什么？',
      ],
      waterpolo: [
        'What are the referee duties?',
        'Berapa lama setiap perlawanan water polo?',
        '水球比赛每节多长时间？',
      ],
      openwater: [
        'What is the minimum water temperature for Open Water?',
        'Bolehkah pelumba memakai wetsuit dalam acara open water?',
        '公开水域比赛的最低水温是多少？',
      ],
      artistic: [
        'How is artistic swimming scored?',
        'Apakah had masa untuk rutin artistik?',
        '花样游泳如何评分？',
      ],
      diving: [
        'How are dives scored?',
        'Apakah darjah kesukaran dalam terjun indah?',
        '跳水的难度系数是什么？',
      ],
      highdiving: [
        'What are the platform height requirements?',
        'Apakah peraturan keselamatan high diving?',
        '高台跳水的平台高度要求是什么？',
      ],
      masters: [
        'What are the age group categories?',
        'Bagaimana rekod masters ditetapkan?',
        '大师组的年龄分组是什么？',
      ],
      paraswimming: [
        'What are the Para Swimming classification classes?',
        'Apakah peraturan permulaan untuk para swimmers?',
        '残疾游泳的分级标准是什么？',
      ],
    }
  ...
  const placeholders = [
    `Ask a ${disciplineNames[discipline] || 'rules'} question...`,
    'Tanya dalam Bahasa Malaysia...',
    '用中文提问...',
    'اسأل بالعربية...',
      ...
      <div className={`min-h-screen bg-gradient-to-b ${bgFrom} via-white to-gray-50 flex flex-col`}>
        <div className="bg-white border-b border-gray-100 px-6 py-4">
          <div className="max-w-4xl mx-auto flex items-center justify-between">
            ...
            <a href="/login" className="text-sm text-blue-600 hover:text-blue-700 font-medium">Already a member? Log in</a>
          </div>
        </div>
        ...
        <div className="flex-1 max-w-3xl mx-auto w-full px-4 py-8">
          <div className="text-center mb-8">
            ...
            <h1 className="text-3xl md:text-4xl font-bold text-gray-900 mb-3">{disciplineNames[discipline]} Rules Assistant</h1>
          ...
          <div className={`bg-white rounded-2xl border ${borderColor} p-6 md:p-8 shadow-sm mb-6`}>
            <h2 className="text-xl font-bold text-gray-900 mb-2 text-center">Ask any {disciplineNames[discipline]} rules question</h2>
            <p className="text-sm text-gray-500 text-center mb-6">Perfect for Technical Officials, coaches, swimmers and parents.</p>

            <div className="space-y-3 mb-6">
              <p className="text-xs font-semibold text-gray-400 uppercase tracking-wide">Example questions:</p>
              {sampleQs.map((q, i) => (
            ...
            <div className={`${isParaSwimming ? 'bg-purple-50 border-purple-100' : 'bg-blue-50 border-blue-100'} border rounded-xl p-4 mb-6`}>
              <p className={`text-sm font-semibold text-center mb-1 ${isParaSwimming ? 'text-purple-900' : 'text-blue-900'}`}>Get 10 FREE questions per month</p>
              <p className={`text-xs text-center ${isParaSwimming ? 'text-purple-700' : 'text-blue-700'}`}>Sign up in seconds — no credit card required.</p>
            </div>
          ...
          <div className="bg-white rounded-2xl border border-gray-100 p-6 mb-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center">
              {['Sign up with your email', `Ask any ${disciplineNames[discipline]} rules question`, 'Get instant answers with rule citations'].map((step, i) => (
                <div key={i}>
                  <div className={`w-10 h-10 ${isParaSwimming ? 'bg-purple-100 text-purple-600' : 'bg-blue-100 text-blue-600'} rounded-full flex items-center justify-center mx-auto mb-2 font-bold`}>{i + 1}</div>
          ...
          <div className="text-center">
            <a href="/pricing" className="text-sm text-blue-600 hover:text-blue-700 font-medium">View all plans and pricing →</a>
          </div>
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="font-semibold text-gray-900 flex items-center gap-2">
                  ...
                  {isParaSwimming && <span className="text-xs bg-purple-100 text-purple-700 px-2 py-0.5 rounded-full font-medium">WPS</span>}
                </h1>
                <p className={`text-xs ${isParaSwimming ? 'text-purple-400' : 'text-gray-400'}`}>
                  {disciplineCodes[discipline]}{isParaSwimming && ' · World Para Swimming (IPC)'}
                </p>
              </div>
            </div>

            {plan !== 'elite' && plan !== 'all_disciplines' && (
              <div className="flex items-center gap-3">
                {plan === 'lite' ? (
                  <div className="text-right">
                    <div className="text-xs text-gray-400">This month</div>
                    <div className="text-sm font-medium text-gray-700">{monthlyRemaining} of 10 left</div>
                    {resetDate && <div className="text-xs text-gray-400">Resets {daysUntilReset}d</div>}
                  </div>
                ) : (
                  <div className="text-right">
                ...
                <div className="w-16 bg-gray-100 rounded-full h-2">
                  <div
                    ...
                    style={{ width: plan === 'lite' ? `${Math.min(((10 - monthlyRemaining) / 10) * 100, 100)}%` : `${Math.min((usage / dailyLimit) * 100, 100)}%` }}
                  />
                </div>
              </div>
            )}
        ...
        {plan === 'lite' && monthlyRemaining <= 0 && (
          <div className="bg-red-50 border-b border-red-100 px-6 py-3">
            <div className="max-w-4xl mx-auto text-center">
              <p className="text-sm font-medium text-red-800 mb-1">You&apos;ve used all 10 free questions this month</p>
              <p className="text-xs text-red-600 mb-2">Resets in {daysUntilReset} days on {resetDate}</p>
        ...
        <div className="flex-1 overflow-y-auto px-6 py-6">
          <div className="max-w-4xl mx-auto space-y-6">
            ...
            {/* Language banner — first time only */}
            {showLangBanner && (
              <div className={`${isParaSwimming ? 'bg-purple-50 border-purple-100' : 'bg-blue-50 border-blue-100'} border rounded-xl px-4 py-3 flex items-start gap-3`}>
                <span className="text-xl flex-shrink-0">🌐</span>
                <div className="flex-1 min-w-0">
                  <p className={`text-sm font-semibold ${isParaSwimming ? 'text-purple-900' : 'text-blue-900'} mb-0.5`}>Ask in any language</p>
                  <p className={`text-xs ${isParaSwimming ? 'text-purple-700' : 'text-blue-700'}`}>
                    AquaRef understands and responds in Bahasa Malaysia, English, Arabic, Chinese, Japanese, and 90+ more languages. Just type naturally.
                  </p>
                </div>
                <button
                  onClick={() => { localStorage.setItem('aquaref_lang_banner_v1', 'seen'); setShowLangBanner(false) }}
                  className={`flex-shrink-0 text-xs ${isParaSwimming ? 'text-purple-400 hover:text-purple-600' : 'text-blue-400 hover:text-blue-600'} font-medium`}
                >
                  Got it ✕
                </button>
              </div>
              ...
              <div className="text-center py-16">
                ...
                {plan === 'lite' && (
                  <div className="mt-3 inline-block bg-green-50 border border-green-200 rounded-lg px-4 py-2">
                    <p className="text-xs text-green-700">LITE Plan — <strong>{monthlyRemaining} of 10</strong></p>
                  </div>
                )}
              ...
              <div key={i} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
                ...
                {msg.role === 'user' ? (
                  <div style={{ backgroundColor: isParaSwimming ? '#7c3aed' : '#1e40af', borderRadius: '16px 16px 4px 16px', padding: '12px 20px', maxWidth: '75%' }}>
                    <p style={{ color: '#ffffff', fontSize: '14px', fontWeight: '600', margin: 0, lineHeight: '1.5' }}>{msg.content}</p>
                  </div>
                ) : (
                  <div className="max-w-3xl w-full">
                      ...
                      <div className="text-gray-700 text-sm">
                        <ReactMarkdown
                          ...
                          components={{
                            ...
                            table: ({ children }) => <div className="overflow-x-auto mb-4 rounded-lg border border-gray-200"><table className="min-w-full text-sm">{children}</table></div>,
                            thead: ({ children }) => <thead className={`${isParaSwimming ? 'bg-purple-50' : 'bg-blue-50'}`}>{children}</thead>,
                            tbody: ({ children }) => <tbody className="divide-y divide-gray-100 bg-white">{children}</tbody>,
                            tr: ({ children }) => <tr className="hover:bg-gray-50 transition-colors">{children}</tr>,
                            th: ({ children }) => <th className={`px-4 py-2.5 text-left text-xs font-semibold ${isParaSwimming ? 'text-purple-700' : 'text-blue-700'} border-b border-gray-200`}>{children}</th>,
                    ...
                    {isShareableMessage(msg.content) && (
                      <div className="flex items-center gap-2 mt-2 ml-1">
                        ...
                        <button onClick={() => handleFeedback(i, 'like')} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs transition-colors ${msg.feedback === 'like' ? 'bg-green-100 text-green-700' : 'bg-gray-100 text-gray-500 hover:bg-green-50 hover:text-green-600'}`}>Helpful</button>
                        <button onClick={() => handleFeedback(i, 'dislike')} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs transition-colors ${msg.feedback === 'dislike' ? 'bg-red-100 text-red-700' : 'bg-gray-100 text-gray-500 hover:bg-red-50 hover:text-red-600'}`}>Not helpful</button>
                        <span className="text-gray-200">|</span>
                        <button onClick={() => handleWhatsAppShare(i)} className="flex items-center gap-1 px-2 py-1 rounded-lg text-xs transition-colors bg-gray-100 text-gray-500 hover:bg-green-50 hover:text-green-600" title="Share to WhatsApp">
                        ...
                        <button onClick={() => handleShareAnswer(i)} className={`flex items-center gap-1 px-2 py-1 rounded-lg text-xs transition-colors ${copiedIndex === i ? 'bg-blue-100 text-blue-700' : 'bg-gray-100 text-gray-500 hover:bg-blue-50 hover:text-blue-600'}`} title={nativeShareSupported ? 'Share' : 'Copy answer'}>
                          {copiedIndex === i ? 'Copied!' : nativeShareSupported ? (
                            <><svg viewBox="0 0 24 24" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><circle cx="18" cy="5" r="3"/><circle cx="6" cy="12" r="3"/><circle cx="18" cy="19" r="3"/><line x1="8.59" y1="13.51" x2="15.42" y2="17.49"/><line x1="15.41" y1="6.51" x2="8.59" y2="10.49"/></svg>Share</>
                          ) : (
                            <><svg viewBox="0 0 24 24" className="w-3 h-3" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><rect x="9" y="9" width="13" height="13" rx="2" ry="2"/><path d="M5 15H4a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h9a2 2 0 0 1 2 2v1"/></svg>Copy</>
                          )}
                        </button>
                      </div>
                    )}
            ...
            <div className="flex gap-3">
              <input
                ...
                placeholder={isLimitReached
                  ...
                  : placeholders[Math.floor(Date.now() / 10000) % placeholders.length]
                }
              ...
              <button
                ...
                className={`text-white px-6 py-3 rounded-xl text-sm font-medium disabled:opacity-50 disabled:cursor-not-allowed ${
                  isParaSwimming ? 'bg-purple-600 hover:bg-purple-700' : 'bg-blue-600 hover:bg-blue-700'
                }`}
              >
                Send
              </button>
