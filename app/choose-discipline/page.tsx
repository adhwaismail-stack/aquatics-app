...
export default function DashboardPage() {
  ...
  useEffect(() => {
    const getUser = async () => {
      ...
      if (files) {
        const live = [...new Set(files.map(f => f.discipline))]
  ...
  const canAccessDiscipline = (disciplineId: string) => {
    if (!subscription) return true // No subscription record = allow access for now (testing)
  ...
  const handleSwitchDiscipline = async () => {
    if (!subscription) return

    // Check if switch is allowed (once per month)
