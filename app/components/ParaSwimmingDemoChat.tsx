...
export default function ParaSwimmingDemoChat(props: Props) {
  ...
  async function ask(question: string) {
    ...
    try {
      const res = await fetch("/api/demo", {
        ...
        body: JSON.stringify({ question: question, discipline: "para-swimming" }),
      });
  ...
  return (
    <div className="max-w-2xl">
      <div className="mb-5 p-3 border border-aqua-pool/30 bg-aqua-pool/10 rounded-lg">
        <p className="text-xs text-aqua-paper/80 leading-relaxed">
          <span className="font-cite tracking-widest text-aqua-pool">WPS / IPC NOTICE — </span>
          Para Swimming is governed by World Para Swimming (WPS) under the International Paralympic Committee (IPC), separate from World Aquatics. Answers cite the World Para Swimming Rules &amp; Regulations (February 2026).
        </p>
      </div>
          ...
          <form onSubmit={handleSubmit} className="bg-white/5 border border-white/10 rounded-lg p-6">
            <div className="flex gap-3 items-center">
              <input
                ...
                placeholder={props.placeholder || "Ask anything about Para Swimming rules..."}
              />
              <button
                type="submit"
