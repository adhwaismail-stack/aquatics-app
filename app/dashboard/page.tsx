  ...
  const [showFeedbackModal, setShowFeedbackModal] = useState(false)
    ...
    // Submit chooser state (Phase B chunk 3)
    const [showSubmitChooser, setShowSubmitChooser] = useState(false)
    const [submitMonthlyEvents, setSubmitMonthlyEvents] = useState(0)
    const [submitMonthlyAnnouncements, setSubmitMonthlyAnnouncements] = useState(0)
    const [submitSuspension, setSubmitSuspension] = useState<{ level: string; reason: string; suspension_until: string | null } | null>(null)
    ...
    const [eventForm, setEventForm] = useState({
      ...
      country: '', state: '', location: '', start_date: '', end_date: '',
    })
  ...
  const [eventSubmitResult, setEventSubmitResult] = useState<{ success: boolean; message: string; slug?: string } | null>(null)
    // Announcement submission form state
    const [annForm, setAnnForm] = useState({
      title: '', description: '', content: '', url: '',
      country: '', state: '', open_new_tab: false,
    })
    const [annThumbnailFile, setAnnThumbnailFile] = useState<File | null>(null)
    const [annAgreed, setAnnAgreed] = useState(false)
    const [annSubmitting, setAnnSubmitting] = useState(false)
  const [annSubmitResult, setAnnSubmitResult] = useState<{ success: boolean; message: string; slug?: string } | null>(null)
  ...
  setLoading(false)
    ...
    const handleLogout = async () => { await supabase.auth.signOut(); window.location.href = '/' }
      const handleOpenSubmitChooser = async () => {
      if (!user?.email) return

      // LITE / no subscription — show upgrade message via chooser modal
      if (subscription?.plan === 'lite' || !subscription) {
        setShowSubmitChooser(true)
        return
      }

      setSubmitChooserLoading(true)
      setShowSubmitChooser(true)
      ...
      try {
        ...
        const { data: violations } = await supabase
          ...
          .select('level, reason, suspension_until, appeal_status')
        ...
        if (violations && violations.length > 0) {
          setSubmitSuspension(violations[0])
        } else {
          setSubmitSuspension(null)
        }

        // Load monthly counts (only relevant for PRO)
        const isPro = subscription?.plan === 'pro' || subscription?.plan === 'starter'
        if (isPro) {
          ...
          setSubmitMonthlyEvents(eventsRes.count || 0)
          setSubmitMonthlyAnnouncements(annsRes.count || 0)
        } else {
          setSubmitMonthlyEvents(0)
          setSubmitMonthlyAnnouncements(0)
        }
      } catch (err) {
        console.error('Failed to load submit chooser data:', err)
      }

      setSubmitChooserLoading(false)
    }
  ...
  const events = (() => {
    ...
    .carousel-scroll::-webkit-scrollbar { display: none; }
        ...
        {showSubmitAnnouncementForm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl">
              <div className="flex items-center justify-between p-6 border-b border-gray-100 flex-shrink-0">
                <div>
                  <h3 className="font-bold text-lg text-gray-900">Submit Announcement</h3>
                  <p className="text-xs text-gray-400 mt-0.5">Reviewed within 24-48 hours</p>
                </div>
                <button onClick={() => { setShowSubmitAnnouncementForm(false); setAnnSubmitResult(null) }} className="text-gray-400 hover:text-gray-600 text-xl px-2">✕</button>
              </div>

              <div className="flex-1 overflow-y-auto p-6">
              {annSubmitResult?.success ? (
                  <div className="text-center py-8">
                    <div className="text-5xl mb-3">✓</div>
                    <p className="font-semibold text-gray-900 mb-2">Submission received!</p>
                    <p className="text-sm text-gray-500 mb-4">{annSubmitResult.message}</p>
                    {annSubmitResult.slug && (
                      <div className="bg-orange-50 border border-orange-100 rounded-lg p-4 mb-6 text-left">
                        <p className="text-xs font-medium text-orange-900 mb-1">Once approved, your announcement will live at:</p>
                        <code className="text-xs text-orange-700 break-all block bg-white px-2 py-1.5 rounded border border-orange-200">
                          aquaref.co/announcements/{annSubmitResult.slug}
                        </code>
                        <p className="text-xs text-orange-500 mt-2">You can share this link with your audience after admin approval.</p>
                      </div>
                    )}
                    <button onClick={() => {
                      setShowSubmitAnnouncementForm(false)
                      setAnnSubmitResult(null)
                      setAnnForm({ title: '', description: '', content: '', url: '', country: userCountry || 'Malaysia', state: '', open_new_tab: false })
                      setAnnThumbnailFile(null)
                      setAnnAgreed(false)
                    }} className="w-full py-2.5 bg-orange-500 text-white rounded-lg text-sm font-medium hover:bg-orange-600">Done</button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {annSubmitResult && !annSubmitResult.success && (
                      <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">{annSubmitResult.message}</div>
                    )}

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Title *</label>
                      <input type="text" value={annForm.title} onChange={(e) => setAnnForm(prev => ({ ...prev, title: e.target.value.slice(0, 200) }))} placeholder="e.g. National Team Selection Results 2026" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 text-gray-900" />
                      <p className="text-xs text-gray-400 mt-0.5">{annForm.title.length}/200</p>
                    </div>

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Description *</label>
                      <textarea value={annForm.description} onChange={(e) => setAnnForm(prev => ({ ...prev, description: e.target.value.slice(0, 500) }))} rows={2} placeholder="Short description shown on the dashboard card (min 10 characters)" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 text-gray-900 resize-y" />
                      <p className="text-xs text-gray-400 mt-0.5">{annForm.description.length}/500</p>
                    </div>

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Full Content <span className="text-gray-400 font-normal">(optional, shown on the announcement page)</span></label>
                      <textarea value={annForm.content} onChange={(e) => setAnnForm(prev => ({ ...prev, content: e.target.value.slice(0, 5000) }))} rows={4} placeholder="Optional longer-form content for users who click into the announcement" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 text-gray-900 resize-y" />
                    </div>

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">URL *</label>
                      <input type="text" value={annForm.url} onChange={(e) => setAnnForm(prev => ({ ...prev, url: e.target.value.slice(0, 500) }))} placeholder="/events/my-event or https://example.com/registration" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-orange-500 text-gray-900" />
                      <p className="text-xs text-gray-400 mt-0.5">Where the announcement leads when clicked. Must start with /, http://, or https://</p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        ...
                        <input
                          type="text"
                          value={annForm.country || userCountry || ''}
                          onChange={(e) => setAnnForm(prev => ({ ...prev, country: e.target.value }))}
                          disabled={!isElite}
                          placeholder="e.g. Malaysia"
                          className={`w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 ${!isElite ? 'bg-gray-50' : 'bg-white'}`}
                        />
                        {!isElite && <p className="text-xs text-gray-400 mt-0.5">PRO tier limited to your country.</p>}
                      </div>
                      <div className="flex items-end pb-1">
                        <label className="flex items-center gap-2 cursor-pointer">
                          <input type="checkbox" checked={annForm.open_new_tab} onChange={(e) => setAnnForm(prev => ({ ...prev, open_new_tab: e.target.checked }))} className="w-4 h-4 rounded border-gray-300 text-orange-500 focus:ring-orange-500" />
                          <span className="text-sm text-gray-700">Open in new tab</span>
                        </label>
                      </div>
                    </div>

                    {(annForm.country === 'Malaysia' || (!annForm.country && userCountry === 'Malaysia')) && (
                      <div>
                        ...
                        <select value={annForm.state} onChange={(e) => setAnnForm(prev => ({ ...prev, state: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white text-gray-900">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Thumbnail Image <span className="text-gray-400 font-normal">(optional, JPG/PNG, max 2MB)</span></label>
                      <input type="file" accept=".jpg,.jpeg,.png" onChange={(e) => {
                        const f = e.target.files?.[0]
                        if (!f) return
                        if (f.size > 2 * 1024 * 1024) { alert('File too large. Max 2MB.'); return }
                        setAnnThumbnailFile(f)
                      }} className="w-full text-xs text-gray-700" />
                      {annThumbnailFile && <p className="text-xs text-orange-600 mt-1">✓ {annThumbnailFile.name} ({(annThumbnailFile.size / 1024).toFixed(0)}KB)</p>}
                      <p className="text-xs text-gray-400 mt-0.5">If no image, a default card will be generated using your title.</p>
                    </div>

                    <label className="flex items-start gap-2 cursor-pointer pt-2">
                      <input type="checkbox" checked={annAgreed} onChange={(e) => setAnnAgreed(e.target.checked)} className="mt-1 w-4 h-4 rounded border-gray-300 text-orange-500 focus:ring-orange-500" />
                      <span className="text-xs text-gray-700">I have read the <a href="/submission-guidelines" target="_blank" className="text-blue-600 underline">Submission Guidelines</a> and confirm this content meets the standards.</span>
                    </label>
                  </div>
                )}
              </div>
              ...
              {!annSubmitResult?.success && (
                <div className="p-6 border-t border-gray-100 flex-shrink-0 flex gap-2">
                  <button onClick={() => { setShowSubmitAnnouncementForm(false); setAnnSubmitResult(null) }} disabled={annSubmitting} className="flex-1 py-2.5 border border-gray-200 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50 disabled:opacity-50">Cancel</button>
                  <button
                    onClick={async () => {
                      // Validation
                      if (!annForm.title.trim() || annForm.title.trim().length < 5) { setAnnSubmitResult({ success: false, message: 'Title must be at least 5 characters.' }); return }
                      if (!annForm.description.trim() || annForm.description.trim().length < 10) { setAnnSubmitResult({ success: false, message: 'Description must be at least 10 characters.' }); return }
                      if (!annForm.url.trim()) { setAnnSubmitResult({ success: false, message: 'URL is required.' }); return }
                      const trimmedUrl = annForm.url.trim()
                      if (!trimmedUrl.startsWith('/') && !trimmedUrl.startsWith('http://') && !trimmedUrl.startsWith('https://')) {
                        setAnnSubmitResult({ success: false, message: 'URL must start with /, http://, or https://' })
                        return
                      }
                      if (!annAgreed) { setAnnSubmitResult({ success: false, message: 'Please confirm you have read the Submission Guidelines.' }); return }
                      if (!user?.email) return
                      ...
                      try {
                        ...
                        if (annThumbnailFile) {
                          const ext = annThumbnailFile.name.split('.').pop()
                          const thumbPath = `announcement-submissions/${user.email}/${Date.now()}_thumb.${ext}`
                          const { data: thumbUpload, error: thumbErr } = await supabase.storage.from('events').upload(thumbPath, annThumbnailFile, { upsert: false })
                          if (thumbErr) throw new Error('Thumbnail upload failed: ' + thumbErr.message)
                          const { data: pub } = supabase.storage.from('events').getPublicUrl(thumbUpload.path)
                          thumbnailUrl = pub.publicUrl
                        }

                        // 2. Submit to API
                        const finalCountry = annForm.country.trim() || userCountry || 'Malaysia'
                        const res = await fetch('/api/submit-announcement', {
                          method: 'POST',
                          ...
                          body: JSON.stringify({
                            ...
                            state: annForm.state,
                            open_new_tab: annForm.open_new_tab,
                            thumbnail_url: thumbnailUrl,
                          })
                        })
                        const data = await res.json()
                    if (data.success) {
                          setAnnSubmitResult({ success: true, message: 'Your announcement has been submitted and is now in review. Approval typically takes 24-48 hours. You\'ll see it appear on the dashboard once approved.', slug: data.slug })
                        } else {
                          setAnnSubmitResult({ success: false, message: data.error || 'Submission failed. Please try again.' })
                        }
                      } catch (err) {
                        setAnnSubmitResult({ success: false, message: err instanceof Error ? err.message : 'Submission failed.' })
                      }
                  ...
                  >
                    {annSubmitting ? 'Submitting...' : 'Submit for Review'}
                  </button>
                </div>
              )}
            </div>
        ...
        {showSubmitEventForm && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] flex flex-col shadow-2xl">
              <div className="flex items-center justify-between p-6 border-b border-gray-100 flex-shrink-0">
                <div>
                  <h3 className="font-bold text-lg text-gray-900">Submit Event</h3>
                  <p className="text-xs text-gray-400 mt-0.5">Reviewed within 24-48 hours</p>
                </div>
                <button onClick={() => { setShowSubmitEventForm(false); setEventSubmitResult(null) }} className="text-gray-400 hover:text-gray-600 text-xl px-2">✕</button>
              </div>

              <div className="flex-1 overflow-y-auto p-6">
            {eventSubmitResult?.success ? (
                  <div className="text-center py-8">
                    <div className="text-5xl mb-3">✓</div>
                    <p className="font-semibold text-gray-900 mb-2">Submission received!</p>
                    <p className="text-sm text-gray-500 mb-4">{eventSubmitResult.message}</p>
                    {eventSubmitResult.slug && (
                      <div className="bg-blue-50 border border-blue-100 rounded-lg p-4 mb-6 text-left">
                        <p className="text-xs font-medium text-blue-900 mb-1">Once approved, your event will live at:</p>
                        <code className="text-xs text-blue-700 break-all block bg-white px-2 py-1.5 rounded border border-blue-200">
                          aquaref.co/events/{eventSubmitResult.slug}
                        </code>
                        <p className="text-xs text-blue-500 mt-2">You can share this link with your audience after admin approval.</p>
                      </div>
                    )}
                    <button onClick={() => {
                      setShowSubmitEventForm(false)
                      setEventSubmitResult(null)
                      setEventForm({ name: '', description: '', discipline: 'swimming', secondary_disciplines: [], country: userCountry || 'Malaysia', state: '', location: '', start_date: '', end_date: '' })
                      setEventPosterFile(null)
                      setEventProofFile(null)
                      setEventAgreed(false)
                    }} className="w-full py-2.5 bg-green-600 text-white rounded-lg text-sm font-medium hover:bg-green-700">Done</button>
                  </div>
                ) : (
                  <div className="space-y-4">
                    {eventSubmitResult && !eventSubmitResult.success && (
                      <div className="bg-red-50 border border-red-200 rounded-lg p-3 text-sm text-red-700">{eventSubmitResult.message}</div>
                    )}

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Event Name *</label>
                      <input type="text" value={eventForm.name} onChange={(e) => setEventForm(prev => ({ ...prev, name: e.target.value.slice(0, 200) }))} placeholder="e.g. National Age Group Championships 2026" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-900" />
                      <p className="text-xs text-gray-400 mt-0.5">{eventForm.name.length}/200</p>
                    </div>

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Description</label>
                      <textarea value={eventForm.description} onChange={(e) => setEventForm(prev => ({ ...prev, description: e.target.value.slice(0, 1000) }))} rows={3} placeholder="Brief description of the event" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-900 resize-y" />
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Primary Discipline *</label>
                        <select value={eventForm.discipline} onChange={(e) => setEventForm(prev => ({ ...prev, discipline: e.target.value, secondary_disciplines: prev.secondary_disciplines.filter(d => d !== e.target.value) }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white text-gray-900">
                      ...
                      <div>
                        ...
                        <input
                          type="text"
                          value={eventForm.country || userCountry || ''}
                          onChange={(e) => setEventForm(prev => ({ ...prev, country: e.target.value }))}
                          disabled={!isElite}
                          placeholder="e.g. Malaysia"
                          className={`w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900 ${!isElite ? 'bg-gray-50' : 'bg-white'}`}
                        />
                        {!isElite && <p className="text-xs text-gray-400 mt-0.5">PRO tier limited to your country. Upgrade to ELITE for global submissions.</p>}
                      </div>
                    </div>

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Secondary Disciplines <span className="text-gray-400 font-normal">(optional, multi-discipline events)</span></label>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-2 p-3 border border-gray-200 rounded-lg bg-gray-50">
                        {disciplines.filter(d => d.id !== eventForm.discipline).map(d => (
                          <label key={d.id} className="flex items-center gap-2 cursor-pointer">
                            <input type="checkbox" checked={eventForm.secondary_disciplines.includes(d.id)} onChange={(e) => {
                              if (e.target.checked) setEventForm(prev => ({ ...prev, secondary_disciplines: [...prev.secondary_disciplines, d.id] }))
                              else setEventForm(prev => ({ ...prev, secondary_disciplines: prev.secondary_disciplines.filter(x => x !== d.id) }))
                    ...
                    {(eventForm.country === 'Malaysia' || (!eventForm.country && userCountry === 'Malaysia')) && (
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">State <span className="text-gray-400 font-normal">(optional)</span></label>
                        <select value={eventForm.state} onChange={(e) => setEventForm(prev => ({ ...prev, state: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm bg-white text-gray-900">
                          <option value="">— Select state —</option>
                          {MALAYSIA_STATES.map(s => <option key={s} value={s}>{s}</option>)}
                        </select>
                      </div>
                    ...
                    <div>
                      ...
                      <input type="text" value={eventForm.location} onChange={(e) => setEventForm(prev => ({ ...prev, location: e.target.value.slice(0, 200) }))} placeholder="e.g. Bukit Jalil Aquatic Centre" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-green-500 text-gray-900" />
                    </div>

                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">Start Date</label>
                        <input type="date" value={eventForm.start_date} onChange={(e) => setEventForm(prev => ({ ...prev, start_date: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900" />
                      </div>
                      <div>
                        <label className="block text-xs font-medium text-gray-700 mb-1">End Date</label>
                        <input type="date" value={eventForm.end_date} onChange={(e) => setEventForm(prev => ({ ...prev, end_date: e.target.value }))} className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm text-gray-900" />
                      </div>
                    </div>

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Event Poster * <span className="text-gray-400 font-normal">(JPG/PNG, max 2MB)</span></label>
                      <input type="file" accept=".jpg,.jpeg,.png" onChange={(e) => {
                        const f = e.target.files?.[0]
                        if (!f) return
                        if (f.size > 2 * 1024 * 1024) { alert('File too large. Max 2MB.'); return }
                        setEventPosterFile(f)
                      }} className="w-full text-xs text-gray-700" />
                      {eventPosterFile && <p className="text-xs text-green-600 mt-1">✓ {eventPosterFile.name} ({(eventPosterFile.size / 1024).toFixed(0)}KB)</p>}
                    </div>

                    <div>
                      <label className="block text-xs font-medium text-gray-700 mb-1">Sanctioning Letter / Proof Document * <span className="text-gray-400 font-normal">(PDF/JPG/PNG, max 5MB)</span></label>
                      ...
                      {eventProofFile && <p className="text-xs text-green-600 mt-1">✓ {eventProofFile.name} ({(eventProofFile.size / 1024).toFixed(0)}KB)</p>}
                      <p className="text-xs text-gray-400 mt-0.5">Required: official sanctioning letter from federation/organising body, or equivalent authorisation. Stored privately, only visible to AquaRef admin.</p>
                    </div>

                    <label className="flex items-start gap-2 cursor-pointer pt-2">
                      <input type="checkbox" checked={eventAgreed} onChange={(e) => setEventAgreed(e.target.checked)} className="mt-1 w-4 h-4 rounded border-gray-300 text-green-600 focus:ring-green-500" />
                      <span className="text-xs text-gray-700">I have read the <a href="/submission-guidelines" target="_blank" className="text-blue-600 underline">Submission Guidelines</a> and confirm this content meets the standards.</span>
                    </label>
                  </div>
                )}
              ...
              {!eventSubmitResult?.success && (
                <div className="p-6 border-t border-gray-100 flex-shrink-0 flex gap-2">
                  <button onClick={() => { setShowSubmitEventForm(false); setEventSubmitResult(null) }} disabled={eventSubmitting} className="flex-1 py-2.5 border border-gray-200 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50 disabled:opacity-50">Cancel</button>
                  <button
                    onClick={async () => {
                      // Validation
                      if (!eventForm.name.trim() || eventForm.name.trim().length < 5) { setEventSubmitResult({ success: false, message: 'Event name must be at least 5 characters.' }); return }
                      if (!eventForm.location.trim()) { setEventSubmitResult({ success: false, message: 'Location is required.' }); return }
                      if (!eventPosterFile) { setEventSubmitResult({ success: false, message: 'Event poster is required.' }); return }
                      if (!eventProofFile) { setEventSubmitResult({ success: false, message: 'Sanctioning letter / proof document is required.' }); return }
                      if (!eventAgreed) { setEventSubmitResult({ success: false, message: 'Please confirm you have read the Submission Guidelines.' }); return }
                      if (!user?.email) return
                      ...
                      try {
                        // 1. Upload poster to public events bucket
                        const posterExt = eventPosterFile.name.split('.').pop()
                        const posterPath = `submissions/${user.email}/${Date.now()}_poster.${posterExt}`
                        const { data: posterUpload, error: posterErr } = await supabase.storage.from('events').upload(posterPath, eventPosterFile, { upsert: false })
                        if (posterErr) throw new Error('Poster upload failed: ' + posterErr.message)
                        const { data: posterUrl } = supabase.storage.from('events').getPublicUrl(posterUpload.path)

                        // 2. Upload proof to private bucket
                        const proofExt = eventProofFile.name.split('.').pop()
                        const proofPath = `${user.email}/${Date.now()}_proof.${proofExt}`
                        const { data: proofUpload, error: proofErr } = await supabase.storage.from('submission-proofs').upload(proofPath, eventProofFile, { upsert: false })
                        if (proofErr) throw new Error('Proof upload failed: ' + proofErr.message)

                        // 3. Submit to API
                        const finalCountry = eventForm.country.trim() || userCountry || 'Malaysia'
                        const res = await fetch('/api/submit-event', {
                          ...
                          body: JSON.stringify({
                            ...
                            secondary_disciplines: eventForm.secondary_disciplines,
                            country: finalCountry,
                            ...
                            proof_url: proofUpload.path, // path in private bucket
                          })
                        })
                        const data = await res.json()
                      if (data.success) {
                          setEventSubmitResult({ success: true, message: 'Your event has been submitted and is now in review. Approval typically takes 24-48 hours. You\'ll see it appear on the dashboard once approved.', slug: data.slug })
                        } else {
                          setEventSubmitResult({ success: false, message: data.error || 'Submission failed. Please try again.' })
                        }
                      } catch (err) {
                        setEventSubmitResult({ success: false, message: err instanceof Error ? err.message : 'Submission failed.' })
                      }
                  ...
                  >
                    {eventSubmitting ? 'Submitting...' : 'Submit for Review'}
                  </button>
                </div>
              )}
            </div>
          ...
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl p-6 max-w-md w-full shadow-2xl">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-lg text-gray-900">Submit to AquaRef</h3>
                <button onClick={() => setShowSubmitChooser(false)} className="text-gray-400 hover:text-gray-600 text-xl">✕</button>
              </div>

              {/* LITE / no subscription — show upgrade message */}
              {(!subscription || subscription.plan === 'lite') ? (
                <>
                  <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-4 mb-4">
                    <p className="text-sm font-medium text-yellow-900 mb-1">Submitting events and announcements is a PRO/ELITE feature</p>
                    <p className="text-xs text-yellow-700">Upgrade to contribute events and announcements to the AquaRef community.</p>
                  </div>
                  <p className="text-xs text-gray-500 mb-4">
                    PRO (RM14.99/month): 5 events + 10 announcements per month.<br />
                    ELITE (RM39.99/month): unlimited submissions.
                  </p>
                  <a href="/pricing" className="block w-full text-center py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">View Plans →</a>
                  <button onClick={() => setShowSubmitChooser(false)} className="block w-full mt-2 py-2.5 border border-gray-200 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50">Cancel</button>
                </>
              ) : submitChooserLoading ? (
                <div className="text-center py-8 text-sm text-gray-400">Loading...</div>
              ) : submitSuspension ? (
                /* Suspended — show suspension reason */
                <>
                  <div className="bg-red-50 border border-red-200 rounded-xl p-4 mb-4">
                    <p className="text-sm font-semibold text-red-900 mb-1">
                      {submitSuspension.level === 'permanent_ban' ? 'Permanently banned from submitting' : 'Currently suspended from submitting'}
                    </p>
                    {submitSuspension.suspension_until && submitSuspension.level !== 'permanent_ban' && (
                      <p className="text-xs text-red-700 mb-2">
                        Suspension ends: {new Date(submitSuspension.suspension_until).toLocaleString('en-MY', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}
                      </p>
                    )}
                    <p className="text-xs text-red-700"><strong>Reason:</strong> {submitSuspension.reason}</p>
                  </div>
                  <p className="text-xs text-gray-500 mb-4">
                    To appeal this decision, contact us via the Contact page within 7 days.
                  </p>
                  <div className="flex gap-2">
                    <a href="/contact" className="flex-1 text-center py-2.5 bg-blue-600 text-white rounded-lg text-sm font-medium hover:bg-blue-700">Contact Us</a>
                    <button onClick={() => setShowSubmitChooser(false)} className="flex-1 py-2.5 border border-gray-200 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50">Close</button>
                  </div>
                </>
              ) : (
                /* PRO / ELITE — show submit options */
                <>
                  <p className="text-sm text-gray-500 mb-4">
                    Choose what you&apos;d like to submit. All submissions are reviewed within 24-48 hours.
                  </p>

                  {(subscription.plan === 'pro' || subscription.plan === 'starter') && (
                    <div className="bg-blue-50 border border-blue-100 rounded-xl p-3 mb-4">
                      <p className="text-xs font-medium text-blue-900 mb-1">PRO monthly usage</p>
                      <div className="flex items-center justify-between text-xs text-blue-700">
                        <span>Events: <strong>{submitMonthlyEvents} / 5</strong></span>
                        <span>Announcements: <strong>{submitMonthlyAnnouncements} / 10</strong></span>
                      </div>
                      <p className="text-[10px] text-blue-500 mt-1">Limits reset on the 1st of each calendar month.</p>
                    </div>
                  )}

                  {(subscription.plan === 'elite' || subscription.plan === 'all_disciplines') && (
                    <div className="bg-yellow-50 border border-yellow-100 rounded-xl p-3 mb-4">
                      <p className="text-xs font-medium text-yellow-900">ELITE — unlimited submissions</p>
                    </div>
                  )}

                  {(() => {
                    const isPro = subscription.plan === 'pro' || subscription.plan === 'starter'
                    const eventsAtLimit = isPro && submitMonthlyEvents >= 5
                    const annsAtLimit = isPro && submitMonthlyAnnouncements >= 10

                    return (
                      <div className="space-y-2 mb-4">
                        <button
                          onClick={() => { setShowSubmitChooser(false); setShowSubmitEventForm(true) }}
                          disabled={eventsAtLimit}
                          className={`w-full p-4 rounded-xl border text-left transition-all ${eventsAtLimit ? 'bg-gray-50 border-gray-200 cursor-not-allowed opacity-60' : 'bg-white border-green-200 hover:border-green-400 hover:shadow-sm cursor-pointer'}`}
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <p className="font-semibold text-gray-900 text-sm mb-0.5">📅 Submit Event</p>
                              <p className="text-xs text-gray-500">
                                {eventsAtLimit ? 'Monthly limit reached. Upgrade to ELITE for unlimited.' : 'Sanctioned competitions, clinics, training camps, courses'}
                              </p>
                            </div>
                            {!eventsAtLimit && <span className="text-green-600 text-sm">→</span>}
                          </div>
                        </button>

                        <button
                          onClick={() => { setShowSubmitChooser(false); setShowSubmitAnnouncementForm(true) }}
                          disabled={annsAtLimit}
                          className={`w-full p-4 rounded-xl border text-left transition-all ${annsAtLimit ? 'bg-gray-50 border-gray-200 cursor-not-allowed opacity-60' : 'bg-white border-orange-200 hover:border-orange-400 hover:shadow-sm cursor-pointer'}`}
                        >
                          <div className="flex items-start justify-between">
                            <div className="flex-1">
                              <p className="font-semibold text-gray-900 text-sm mb-0.5">📢 Submit Announcement</p>
                              <p className="text-xs text-gray-500">
                                {annsAtLimit ? 'Monthly limit reached. Upgrade to ELITE for unlimited.' : 'Results, club news, official notices, calls for officials'}
                              </p>
                            </div>
                            {!annsAtLimit && <span className="text-orange-600 text-sm">→</span>}
                          </div>
                        </button>
                      </div>
                    )
                  ...
                  <p className="text-xs text-gray-400 mb-3">
                    By submitting, you agree to our <a href="/submission-guidelines" target="_blank" className="text-blue-600 hover:text-blue-700 underline">Submission Guidelines</a>.
                  </p>

                  <button onClick={() => setShowSubmitChooser(false)} className="w-full py-2.5 border border-gray-200 text-gray-600 rounded-lg text-sm font-medium hover:bg-gray-50">Cancel</button>
                </>
              )}
            </div>
          </div>
